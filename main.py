...
def parse_args():
  ...
  parser.add_argument(
        '--wandb_project', type=str, default='multigrid-multi-agent',
...
# Implement and import a MetaController class!
def get_metacontroller_class(config, envs, device):
    return MultiAgent(config, envs, device)
...
def initialize(mode, env_name, debug, visualize, seed, with_expert, wandb_project):
    ...
    envs = utils.make_env(config)
    envs.reset()
    envs.action_space.seed(config.seed)
    envs.observation_space.seed(config.seed)

    metacontroller_class = get_metacontroller_class(config, envs, device)

    return device, config, envs, metacontroller_class

def main(args):
    device, config, envs, metacontroller_class = initialize(
    ...
    if not args.debug and not args.visualize:  # Real run that logs to wandb
      ...
      current_wandb_projects = ['multigrid-multi-agent']  # Add your wandb project here
    ...
    if args.visualize:
      agent = metacontroller_class #(config, envs, device, with_expert=args.with_expert, training=False)
      agent.load_models(model_path=args.load_checkpoint_from)
      agent.visualize(envs.envs[0], args.mode, args.video_dir)
    ...
    agent.train(envs.envs[0], config, device)
