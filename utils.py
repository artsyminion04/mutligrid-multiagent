...
def make_env(config):
    if 'MultiGrid' in config.domain:
        import ppo
        envs = gym.vector.SyncVectorEnv([ppo.make_env(config.domain, config.seed, config.minigrid_mode, config.n_agents, fully_observed=True)])
        # from envs import gym_multigrid
        # from envs.gym_multigrid import multigrid_envs
        # env = gym.make(config.domain, minigrid_mode=config.minigrid_mode, n_agents = config.n_agents, fully_observed=True)
    else:
...
def generate_parameters(mode, domain, debug=False, seed=None, with_expert=None, wandb_project=None):
    ...
    if seed:
        config_mode['seed'] = seed

    config_mode['experiment_name'] = 'MultiGrid'  # TODO: change me

    # Merge configs
    config = dotdict(merge_configs(config_mode, config_domain))

    config["mode"] = mode
    config["load_model_start_path"] = ""
...
def plot_single_frame(frame_id, full_env_image, agents_partial_images, actions, rewards, action_dict,
    ...
    cum_collective_return = np.cumsum(collective_return, axis=0)
    cum_collective_return = np.sum(cum_collective_return, axis=1)
    ...
    for i in range(n_agents):
        ...
        cum_return = np.cumsum(rewards[:,0,i])
        ...
        r_text = 'R_t={}: {}'.format(frame_id, rewards[frame_id,0,i])
        fig.text(text_horizontal_loc, text_vertical_loc-0.1, r_text, fontsize=10)
        if frame_id > 0:
            r_prev_text = 'R_t={}: {}'.format(frame_id-1, rewards[frame_id-1, 0, i])
